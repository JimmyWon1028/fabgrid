...
server.listen(port, '127.0.0.1', async function() {
  try {
    ...
    const standaloneBundlePattern = /^(?:fabLoader(?:\.min)?\.js|fabui\.(?:diagram|lite|gantt|scheduler|htmleditor)(?:\..+)?)$/;
    ...
    if (fs.existsSync(path.join(root, 'dist', 'theme', 'images-mono')) ||
      ...
      throw new Error('Smoke assertions failed: Mono icons must only use dist/theme/mono.');
    }
