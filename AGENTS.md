...
- `fabLoader` 是獨立 browser global，原始碼位於 `src/fabloader/fabLoader.js`，不依賴其他套件，也不附加到 `fabui`。提供 script、CSS、圖片預載、共用文字／HTML／XML 原文快取，以及 HTML 掛載並依序執行 script；`setConfig()` 在使用前集中設定 `script`／`css`／`image`／`text` 四個桶，未設定時都使用 30 秒 timeout，文字 credentials 預設 `'same-origin'`，`getConfig()` 必須回傳獨立副本。會影響資源身份的 options 必須納入快取鍵，所有失敗、逾時與取消路徑都必須清除該筆快取；`cancel(bucket?, url?)` 可取消單一 URL、單桶或全部 pending 載入。`preloadImage(url)` 共用網路預載但每次回傳不同 `<img>`；`preloadImage([url, ...])` 平行預載並回傳真正的 Array，`preloadImage({ name: url })` 平行預載並保留名稱；兩種集合的索引或名稱都必須是可重複讀取的 getter，每次讀取回傳來源相同但 DOM identity 不同的 `<img>`，讓同一 `myImages[index]` 或 `myImage.name` 可直接 append 到不同位置。`loadText()` 非同步載入並保存文字，`getText()` 只同步讀取已完成快取；`loadHtml(url)` 是共用相同快取的 HTML 相容名稱，並比照 `preloadImage()` 支援 `loadHtml([url, ...])` 平行回傳文字陣列及 `loadHtml({ name: url })` 平行回傳同名文字物件，`getHtml()` 維持單一 URL 的同步快取讀取。公開快取清理 API 統一為 `clearTextCache()`，不得保留 `clearHtmlCache()`；`loadXml()` 每次由快取原文解析並回傳新的 `XMLDocument`，格式錯誤時 reject 並清除該筆文字快取。從頂層 `style()`、`script()`、`module()`、`vue()`、`react()`、`run()` 或其相容別名 `wait()` 開始時會自動建立獨立的內部 Promise 佇列，後續可鏈接 `done()` 與 `catch()`，不強制先寫 `queue()`；頂層與鏈式 `wait(callback)` 都必須是 `run(callback)` 的同一函數別名。`style(url)` 載入單一 CSS，`style([url, ...])` 平行載入一組 CSS。`script(url)` 是循序步驟，`script([url, ...])` 會平行載入陣列內互不依賴的 scripts，並在全部完成後才繼續下一步；陣列內執行順序不保證。`vue(url)` 是選用的 Vue 2 SFC 佇列步驟，只有呼叫時才檢查 `System.import()`、Vue 2 完整版與 template compiler；`react(url)` 是選用的 React JSX 佇列步驟，只有呼叫時才檢查 `System.import()`、React 與提供 `createRoot()` 的 ReactDOM Client，再將 JSX 交給頁面既有的 SystemJS loader。fabLoader 不內建或自動下載 Vue、React 或 SystemJS，未使用 `.vue()`／`.react()` 時維持零外部依賴。Demo2 的 `systemjs.config.js` 必須保留為 Vue 設定，React JSX 設定獨立放在 `runtime.config.js` 並以 `// for react` 標記。舊 `.styles()` 不保留。`queue()` 只保留為明確建立空佇列的選用 API。先由正式版 Diagram Demo 與獨立 `demo2/` 試用，方向確認前不得併入 FabUI core。
