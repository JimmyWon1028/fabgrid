...
- 本機 `filterRules` 除標準 operator 名稱外，也必須接受舊 OA/ERP 使用的 `..%`、`%..%`、`%..`，並分別正規化為 `starts`、`contains`、`ends`。中文輸入法 composition 期間的 Search Row 方向鍵與 keyCode `229` 必須保留給候選字操作，不得切換 Grid 焦點；composition 結束後才依完成文字套用篩選。
