...
- fabLoader 是上述規則的唯一例外：只要修改 fabLoader 或 fabDom 原始碼，或修改其共用 build 邏輯，完成後必須自動執行 `npm run build:loader`，而且只允許更新 `dist/fabLoader.js` 與 `dist/fabLoader.min.js`；不得連帶執行其他 build、產生獨立 fabDom dist 或改寫其他 `dist/` 輸出。
- Build 命令固定依照以下範圍執行：
  ...
  - `build loader`：執行 `npm run build:loader`，只產生 `dist/fabLoader.js` 與 `dist/fabLoader.min.js`；`build loader min`：執行 `npm run build:loader -- min`，只產生並保留 `dist/fabLoader.min.js`。fabLoader 不屬於 `fabui.*`，不納入 `build all` 或逗號 scope，也不得產生 ESM。
  ...
  - 所有編譯命令都可在尾端加上 `min`，包含 `build min`、`build lite min`、`build diagram min`、`build gantt min`、`build scheduler min`、`build htmleditor min`、`build loader min`、`build theme min` 與 `build all min`。加上 `min` 時，只產生並保留該範圍內檔名含 `.min.` 的壓縮版 JS／CSS，不得產生或保留同範圍的非壓縮輸出；`build htmleditor min` 只產生並保留 `dist/fabui.htmleditor.min.js` 與 `dist/fabui.htmleditor.min.css`，`build loader min` 只產生並保留 `dist/fabLoader.min.js`；theme 所需圖片依賴仍須保留。`build clear` 不適用 `min`。
...
- FabGrid `select(row, col?)` 的 Row 或 Column index 不在目前可用範圍時必須回傳 `false` 且不做任何動作；不得將負數、超出範圍或非整數 index clamp 成第 0 列或最後一列，也不得改變 active cell、selected row、事件或畫面。
...
- fabLoader／fabDom 合併後契約：`dist/fabLoader.{js,min.js}` 必須內建 `src/fabdom/fabDom.js` 並公開 `fabLoader.dom()`；頁面沒有 jQuery 且 `$` 尚未被使用時安全提供 `$` 別名，既有 jQuery 或其他 `$` 不得覆蓋。`useDom()` 必須優先回傳有效的 `global.jQuery`，否則回傳內建 `fabLoader.dom`，供呼叫端保存為局部 `$`；不得修改或搶占目前全域 `$`。`dom(target).load(url, options?, callback?)` 採 jQuery-like collection 語法並回傳原 collection，實際請求、快取、timeout 與片段 script 執行只可橋接既有 `mountHtml()`。不得產生獨立 `dist/fabDom.{js,min.js}`；Demo2 只載入 fabLoader，未來整合 FabUI 時公開名稱固定為 `fabui.loader` 與 `fabui.dom`。
...
- Cell editor 以 `Enter`／`Tab` 或 `ArrowUp`／`ArrowDown` 移到下一個可編輯 cell 時，不得套用公開 `select()` 的首列對齊行為；active editor 超出可視區後每次只捲動所需列數，並保持在可視區頂部或底部邊界。
...
- 每次使用者明確要求 build 時，依「開發工作流程」定義的 `build`／`build fabui`、`build lite`、`build diagram`、`build gantt`、`build scheduler`、`build htmleditor`、`build loader`、`build theme`、`build clear`、`build all`、逗號分隔多範圍或上述命令的 `min` 形式執行對應操作與檢查；逗號左右不得有空白，`min` 必須套用到清單中的每個範圍。Loader 不納入逗號分隔多範圍與 `build all`；Vue 2 與 FabGrid jQuery wrapper 暫緩且不納入 build gate。
