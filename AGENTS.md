...
- FabUI core 內部使用的原生 `button`、`input`、`label`、`select`、`textarea` 與 `<a>` 必須具有固定 component class，並由 `src/core/native-controls.css` 與 component selector 隔離一般 tag selector；19 組 theme、built-in icon、selected／disabled／readonly 狀態的覆寫權重必須保持有效。此契約目前只涵蓋 FabUI core，不自動擴及 Diagram、Gantt、Scheduler 與 HtmlEditor 等獨立附加元件。
