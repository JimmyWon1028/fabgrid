...
- 回答 FabGrid／FabUI 的預設值、API 契約、相容別名或實際行為（例如 `autoUnmask`）前，必須先查閱目前原始碼與相關測試，必要時再對照文件；不得只憑記憶、舊工作紀錄、文件或推測回答。若原始碼、測試與文件不一致，必須明確指出差異，不得自行斷言。
...
- fabLoader 是上述規則的唯一例外：只要修改 fabLoader 或 fabDom 原始碼，或修改其共用 build 邏輯，完成後必須自動執行 `npm run build:fabloader`，而且只允許更新 `dist/fabLoader.js` 與 `dist/fabLoader.min.js`；不得連帶執行其他 build、產生獨立 fabDom dist 或改寫其他 `dist/` 輸出。
- Build 命令固定依照以下範圍執行：
  ...
  - `build fabloader`：執行 `npm run build:fabloader`，將 `src/fabdom/fabDom.js` 與 `src/fabloader/fabLoader.js` 合併，產生 `dist/fabLoader.js` 與 `dist/fabLoader.min.js`；`build fabloader min`：執行 `npm run build:fabloader -- min`，只產生並保留含 fabDom 的 `dist/fabLoader.min.js`。
  - `build loader`：執行 `npm run build:loader`，只編譯 `src/fabloader/fabLoader.js`，不包含 fabDom，產生 `dist/fabLoader.js` 與 `dist/fabLoader.min.js`；`build loader min`：執行 `npm run build:loader -- min`，只產生並保留不含 fabDom 的 `dist/fabLoader.min.js`。fabLoader 與 loader scope 都不屬於 `fabui.*`，不納入 `build all` 或逗號 scope，也不得產生 ESM。
  - `build theme`：執行 `npm run build:theme`，只編譯 `dist/theme/` 下完整 17 組 theme CSS 與圖片依賴，包含 `fabui.default.css` 與 `fabui.default.min.css`，不得重新編譯或刪除其他 `dist/` 輸出；`build theme min` 執行 `npm run build:theme -- min`，只保留 17 組 `.min.css`，包含 `fabui.default.min.css`。
  - `build locale`：執行 `npm run build:locale`，只編譯 `dist/locales/` 下的 `fabui-locale.en.{js,min.js}`、`fabui-locale.zh-TW.{js,min.js}` 與 `fabui-locale.zh-CN.{js,min.js}`，不得重新編譯或刪除其他 `dist/` 輸出；`build locale min` 執行 `npm run build:locale -- min`，只保留三個 `.min.js` 語言檔。
  - `build clear`：只清除 `dist/` 下的所有檔案與資料夾並保留 `dist/` 目錄本身，不得執行任何編譯。
  - `build all`：依序執行 `npm run build`、`npm run build:lite`、`build diagram`、`npm run build:gantt`、`npm run build:scheduler`、`npm run build:htmleditor`、`npm run build:locale`、`npm run build:vue`、`npm run build:jquery`、`npm run build:fabui-jquery`，編譯以上全部發佈輸出與三個 wrapper；一般 `build` 已包含 theme，因此不重複執行 `build theme`，也不得額外執行 `build clear`。
  - 可用半形逗號一次指定多個編譯範圍，格式固定為 `build <scope>,<scope> [min]`，逗號左右不得有空白；例如 `build fabui,htmleditor min` 必須依序編譯 FabUI core 與 HtmlEditor，並對兩個範圍都套用 `min`。可組合的 scope 為 `fabui`、`lite`、`diagram`、`gantt`、`scheduler`、`htmleditor`、`theme`、`locale`，依使用者列出的順序各執行一次；重複 scope 只執行第一次。`all` 與 `clear` 必須單獨使用，不得放入逗號清單，也不得和其他 scope 組合。
  - 所有編譯命令都可在尾端加上 `min`，包含 `build min`、`build lite min`、`build diagram min`、`build gantt min`、`build scheduler min`、`build htmleditor min`、`build fabloader min`、`build loader min`、`build theme min`、`build locale min` 與 `build all min`。加上 `min` 時，只產生並保留該範圍內檔名含 `.min.` 的壓縮版 JS／CSS，不得產生或保留同範圍的非壓縮輸出；`build htmleditor min` 只產生並保留 `dist/fabui.htmleditor.min.js` 與 `dist/fabui.htmleditor.min.css`，`build fabloader min` 與 `build loader min` 都只產生並保留各自模式的 `dist/fabLoader.min.js`；theme 所需圖片依賴仍須保留。`build clear` 不適用 `min`。
- 所有 build 命令一律不得產生 `.esm.js`、`.esm.min.js` 或其他 ESM 發佈檔；只輸出 browser global JavaScript、壓縮 JavaScript、CSS、壓縮 CSS 與必要 theme／圖片資產。
- Diagram、Gantt、Scheduler、HtmlEditor、Vue 2、FabGrid jQuery 與 FabUI jQuery 必須保持獨立 bundle，不得併入 `fabui.*` core。一般 `build` 不編譯 wrapper；`build all` 必須在 locale 之後依序執行 `npm run build:vue`、`npm run build:jquery` 與 `npm run build:fabui-jquery`。三個 wrapper 仍可使用各自的獨立 build 命令。
...
- FabUI 使用的 icon 定義統一放在 `src/fabui.icon.css`，例如 `icon-datebox`、`icon-refwin`；不要在核心 CSS 直接硬編圖檔路徑。外部透過 `iconCls` 傳入的普通 icon class 必須能覆蓋 FabUI 子元素 reset，不得要求外部宣告加上 `:root` 或 `!important`；`src/core/native-controls.css` 的 component descendant reset 必須維持低權重 `:where(...)`。
- FabUI core 內部使用的原生 `button`、`input`、`label`、`select`、`textarea` 與 `<a>` 必須具有固定 component class，並由 `src/core/native-controls.css` 與 component selector 隔離一般 tag selector；17 組 theme、built-in icon、selected／disabled／readonly 狀態的覆寫權重必須保持有效。此契約目前只涵蓋 FabUI core，不自動擴及 Diagram、Gantt、Scheduler 與 HtmlEditor 等獨立附加元件。
...
- `demo/dev-grid.html` 與 `demo/grid.html` 的 `editMode` 預設為 `false`；工具列勾選「編輯」只將 `allowEditing` 設為 `true`，`editOnSelect` 維持 `false`，因此單擊只選取 cell，雙擊、Enter 或 F2 才進入編輯。
...
- FabUI core 與獨立附加元件只內建英文；`en`、繁中與簡中語言檔集中於 `src/locales/fabui-locale.en.js`、`src/locales/fabui-locale.zh-TW.js`、`src/locales/fabui-locale.zh-CN.js`，需要時才載入。未載入任何語言檔時預設英文；英文語言檔用於明確切回內建英文。語言包載入後必須自動設為全域顯示語言並更新既有控件；指定未載入語言時必須回退英文。新增任何核心 UI 文字時，必須同步補齊 `en`、`zh-TW`、`zh-CN` locale key；三組 locale pack 的 key 必須一致，且 `zh-Hant`／`zh_Hant_TW` 要正規化為 `zh-TW`、`zh-Hans`／`zh_CN` 要正規化為 `zh-CN`。demo-only 文字若會隨語言切換，也要放進 demo locale pack，不要寫死單一語言。
...
- 所有既有與未來新增的 popup 都必須支援按 `Escape` 與點擊 popup 外部關閉；點擊 popup 內部或其 trigger 不得誤關閉。關閉只負責收起 popup，不得隱含套用、清除或提交尚未確認的內容；同時開啟多個 popup 時，點進其中一個也必須關閉其餘 popup。Popup 所需的 document／window listener 只在開啟期間綁定，關閉或 dispose 時必須立即解除。掛載到頁面層的 popup 必須動態高於 Grid 所在視窗的 stacking level，不得硬編特定外部視窗套件的 z-index。
...
- FabGrid `getExcelBlob()`／`exportExcel()` 的第二參數支援 `{ sheetName, visibleOnly }`，並保留舊 boolean 寫法；`fabui.Excel.getBlob()`／`fabui.Excel.export()` 以 `sheets: [{ name, grid, visibleOnly }]` 將多個 Grid 匯出為同一 XLSX 的多個工作表。工作表名稱必須處理 Excel 禁用字元與 31 字元上限，重複名稱依序加上數字後綴。
...
- FabGrid 所有公開方法的數字欄位參數統一使用完整 `grid.columns` index 並計入隱藏欄位，十進位整數字串視為相同 index；Column `binding`／`name`／`header` 必須以字母開頭，避免和數字字串衝突。此規則包含 `getColumn()`、`getCellData()`、`setCellData()`、`toggleSort()`、`select()`、`selectRange()`、`selectRow()`、`scrollIntoView()`、兩種 `startEditing()`、欄位篩選與欄位顯示 API；需要畫面 cell 的方法指定隱藏欄位時回傳 `false`，資料讀寫則仍可操作隱藏欄位。Grid 內部可繼續使用 `visibleColumns` index，但不得直接傳入上述公開方法。
...
- FabGrid Column `allowResizing` 預設為 `true`；設為 `false` 時不得顯示該欄 resize 操作區，滑鼠拖曳、雙擊 AutoFit 與 `autoSizeColumn()` 都不得改變欄寬或觸發 resize／AutoFit 事件。Grid 全域 `allowResizing` 仍只控制使用者操作，不得改變既有程式呼叫 `autoSizeColumn()` 的行為。
...
- FabGrid `columns` 可用巢狀 `columns` 定義多層合併 Header；群組節點只提供 `header`、`align` 與子 `columns`，`grid.columns`／`visibleColumns` 仍保持扁平葉欄位集合。未群組的葉欄位要縱向合併到最深 Header 列，Search Row 位於所有 Header 列下方；隱藏子欄時群組寬度依剩餘可見葉欄重算，全部隱藏時不渲染群組。群組 Header 不得觸發排序、篩選或 resize；Column drag 只能在相同直屬群組內重排。`hitTest()` 對群組與縱向合併 Header 必須回傳 `mergedRange`。
- FabGrid Column `cssClass` 預設為 `null`；字串可包含一個或多個 CSS class，必須套用到該欄所有 body 資料 cell，但不得套用到 Header。初始化前可直接在 Column definition 設定。
- FabGrid `FabGrid.CellMaker.makeLink(options)` 必須回傳可直接指定給 Column `cellTemplate` 的 function，建立 `<a class="fg-cell-maker">`；`text`、`href`、`title` 支援 template string 或 callback，`click(event, ctx)` 執行前取消預設導覽。連結預設使用 `--fg-link-text`，選取 cell 時繼承選取文字色。
- FabGrid body cell 內容未超過可用寬度時必須保留正常左右 padding；實際內容被截斷時只移除右側 padding，左側 padding 與未超長 cell 對齊不得改變。CellMaker link 與 TreeGrid 內容也必須依實際內容寬度判斷。
- FabGrid Row hover 必須直接切換既有 body、凍結欄、Row Header 與 selection cell 的 `fg-row-hovered` class；跨列或離開 Grid 不得因此呼叫 `renderVisibleRows()` 重建可視 DOM。
- FabGrid Header 文字與既有 Filter icon 空間本身不足時，才將左右外側間距縮為 `2px`；若兩者原本放得下，切換排序前後必須維持原左側間距。啟用 filter 時 Filter icon 必須持續顯示，切換排序前後不得改變其位置；未啟用 filter 時不得保留其位置，文字可以使用該空間。Sort icon 預設緊接文字右側，加入 Sort icon 後寬度不足且同時顯示 Filter icon 時必須移到 Filter icon 下方，不得覆蓋文字。
- FabGrid `clearSort()` 必須一次清除所有單欄／多欄排序；目前沒有排序時回傳 `false` 且不重新套用 view。成功時回傳 `true`，本機與 CollectionView 立即恢復原始順序；遠端模式回到第 1 頁並只重新載入一次。
...
- FabGrid 唯讀 `selectedRow` 必須回傳目前 active cell 所在列的 `Row`／`GroupRow` instance，沒有可用列時回傳 `null`；列索引與資料分別由 `.index` 與 `.dataItem` 取得。此屬性跟隨 active cell，不等同 `selectedRows` 的列選取狀態，`unselectRow()` 清除列選取後仍可保留目前 active row。
...
- FabGrid `select(row, col?)` 的 `col` 必須對應完整 `grid.columns` index 並計入隱藏欄位；指定隱藏欄位，或 Row／Column index 不在目前可用範圍時，必須回傳 `false` 且不做任何動作。不得將負數、超出範圍或非整數 index clamp 成第 0 列或最後一列，也不得改變 active cell、selected row、事件或畫面。
- FabGrid 所有公開事件參數中的 `e.col`、`e.col2`、`e.activeCol` 與 `e.anchorCol` 必須對應完整 `grid.columns` index 並計入隱藏欄位；內部可見欄 index 不得直接暴露為上述欄位，需要公開時使用 `e.viewCol`、`e.viewCol2`、`e.viewActiveCol`、`e.viewAnchorCol` 或 `e.viewRange`。`formatItem`、選取、剪貼簿與編輯事件必須遵循同一契約。
- FabGrid `selectionChanging`／`selectionChanged` 必須固定包含完整 active、anchor、range 與 `view*` 座標，另包含 `changedRow`、`selected`、`allRows`；一般 cell 選取時前兩者為 `null`、`allRows` 為 `false`，列勾選／取消時以 `changedRow` 與 `selected` 表示變更列，全部列操作使用 `allRows: true`。
- FabGrid 在 `Cell` 與 `CellRange` 模式點擊 RowHeader 都必須建立整列選取；RowHeader 保持原本外觀並設定 `aria-selected="true"`，資料 cell 顯示完整整列範圍。`Ctrl/Cmd + C`、`getSelectedText()` 與 `getClipString()` 必須依畫面順序複製該列所有可見欄位，不含標題與隱藏欄位；點擊一般 cell 後清除 RowHeader 整列選取，editor 開啟時仍只複製 editor 選取文字。
...
- FabGrid 本機、CollectionView 與 TreeGrid 多欄排序必須先為每列準備各欄排序值，再進入 comparator；不得在 comparator 的每次比較中重複讀取 binding、轉換 Date、number 或 lowercase 字串。`dataType: 'number'` 的字串值若含半形千位逗號或空白，必須移除群組符號後依實際數值排序，不得退回文字順序；排序結果、null 順序與其他既有 data type 語意必須保持相容。
- FabGrid `dataType: 'number'` 的有限數值字串必須在 Grid 內部統一正規化，供 cell 顯示、排序、Search Row／Excel-like 篩選、aggregate 與 XLSX 匯出使用；不得修改 `itemsSource`，未格式化的 `getCellData()` 仍回傳原值。無效文字維持原樣顯示但排除於數值運算，number editor 提交後則寫入 Number。
- FabGrid 點擊 Header 或呼叫 `toggleSort()` 排序後可以將垂直捲動回到第一列。直接呼叫 `toggleSort()` 必須保留 `bodyScroll.scrollLeft`；點擊 Header 時，已完整可見的非凍結欄也必須保留水平位置，只有該欄部分被凍結區或右側邊界遮住時，才可做最小幅度的水平調整讓整欄可見，不得跳回水平起點。
- FabGrid `frozenColumns` 與 `frozenRightColumns` 必須依完整欄位順序決定左右凍結範圍；`visible: false` 的 hidden 欄位仍計入指定數量，但不產生可見凍結 cell，也不得讓凍結範圍向後多吃可見欄位。
...
- FabGrid `syncScrollRender` 預設為 `false`，同一 animation frame 內的連續 scroll event 必須合併為一次 scroll-only render；凍結欄、右凍結欄、Row Header、selection pane 與自訂 scrollbar thumb 必須在 render 前先跟隨目前原生 scroll 位置。設為 `true` 時才在 scroll event 內同步 render。
- FabGrid Footer aggregate 必須依目前 `dataView` 世代快取；`applyView()`、cell 編輯寫值、`setCellData()` 與 `invalidate()` 後使快取失效並標記 dirty，下一次 render 必須只額外重畫 Footer，不得因純垂直 render 漏掉更新或連帶重建 Header。公式直接更新其他 cell（包含 `isReadOnly` 欄位）後呼叫 `invalidate()` 也必須重算所有 Footer aggregate。一般資料未變更的 render／selection 沿用快取；`refreshFooter()` 清除快取並重畫 Footer，供依賴外部狀態的自訂 aggregate 手動更新。`column.footer` function／固定值不納入 aggregate 快取。
- FabGrid `footerRows` 定義多列 Footer，`footerHeight` 是每列高度；未設定時必須維持既有單列 `footerLabel`、`column.footer` 與 `aggregate` 行為。Footer API 與 `columnFooters`／`bottomLeftCells` panel 必須同時支援列 index 或 `key`；`formatItem.e.row`、hitTest、AutoFit 與 Excel 匯出都必須保留實際 Footer 列 index。Footer 的有限數字值預設靠右並顯示千位分隔，明確的 `footerFormatter` 仍優先。
- FabGrid 群組列與群組 Footer 的數字 aggregate 預設顯示千位分隔，明確的 `footerFormatter`／`formatter` 仍優先，且不得修改 Column 本身的 `thousandsSeparator` 設定。
...
- fabLoader 佇列式 `style()`、`script()`、`module()`、`vue()` 與 `react()` 的載入錯誤只輸出至 `console.error` 並繼續後續步驟；直接載入方法仍維持 Promise reject，佇列 `catch()` 只處理 `run()`／`done()` callback 例外。
- `build fabloader` 的 fabLoader／fabDom 合併契約：`dist/fabLoader.{js,min.js}` 必須內建 `src/fabdom/fabDom.js` 並公開 `fabLoader.dom()`；`build loader` 的同名輸出只包含 `src/fabloader/fabLoader.js`，不得內建 fabDom。頁面沒有 jQuery 且 `$` 尚未被使用時，只有合併版會安全提供 `$` 別名，既有 jQuery 或其他 `$` 不得覆蓋。`useDom()` 必須優先回傳有效的 `global.jQuery`，其次回傳可用的 `global.fabDom`；兩者皆無時拋出錯誤，不得修改或搶占目前全域 `$`。`dom(target).load(url, options?, callback?)` 採 jQuery-like collection 語法並回傳原 collection，實際請求、快取、timeout 與片段 script 執行只可橋接既有 `mountHtml()`。不得產生獨立 `dist/fabDom.{js,min.js}`；Demo2 使用合併版，未來整合 FabUI 時公開名稱固定為 `fabui.loader` 與 `fabui.dom`。
- `fabLoader` 是獨立 browser global，原始碼位於 `src/fabloader/fabLoader.js`，不依賴其他套件，也不附加到 `fabui`。提供 script、CSS、圖片預載、共用文字／HTML／XML 原文快取，以及 HTML 掛載並依序執行 script；`setConfig()` 在使用前集中設定 `script`／`css`／`image`／`text` 四個桶，未設定時都使用 30 秒 timeout，文字 credentials 預設 `'same-origin'`，已完成的文字快取以 `maxEntries: 100` 執行 LRU 淘汰，`0` 表示不保留完成紀錄，`getConfig()` 必須回傳獨立副本。會影響資源身份的 options 必須納入快取鍵，所有失敗、逾時與取消路徑都必須清除該筆快取；`cancel(bucket?, url?)` 可取消單一 URL、單桶或全部 pending 載入。`clearResourceCache(bucket?, url?)` 只清除已完成的 Script／CSS／圖片去重紀錄並回傳筆數，不取消 pending、不移除既有 DOM，文字仍只用 `clearTextCache()`。`preloadImage(url)` 共用網路預載但每次回傳不同 `<img>`；`preloadImage([url, ...])` 平行預載並回傳真正的 Array，`preloadImage({ name: url })` 平行預載並保留名稱；兩種集合的索引或名稱都必須是可重複讀取的 getter，每次讀取回傳來源相同但 DOM identity 不同的 `<img>`，讓同一 `myImages[index]` 或 `myImage.name` 可直接 append 到不同位置。`loadText()` 非同步載入並保存文字，`getText()` 只同步讀取已完成快取；`loadHtml(url)` 是共用相同快取的 HTML 相容名稱，並比照 `preloadImage()` 支援 `loadHtml([url, ...])` 平行回傳文字陣列及 `loadHtml({ name: url })` 平行回傳同名文字物件，`getHtml()` 維持單一 URL 的同步快取讀取。公開文字快取清理 API 統一為 `clearTextCache()`，不得保留 `clearHtmlCache()`；`loadXml()` 每次由快取原文解析並回傳新的 `XMLDocument`，格式錯誤時 reject 並清除該筆文字快取。`mountHtml()` 必須以最終 response URL 改寫 `src`／`href`／`action`／`poster`／`srcset`／`formaction`／`object[data]` 與 inline style `url()`；下載及外部 script／module 載入失敗會 reject 且已插入 DOM 保留，inline classic script runtime error 交由瀏覽器錯誤機制處理，不保證進入 Loader Promise／queue `.catch()`。從頂層 `style()`、`script()`、`module()`、`vue()`、`react()`、`run()` 或其相容別名 `wait()` 開始時會自動建立獨立的內部 Promise 佇列，後續可鏈接 `done()` 與 `catch()`，不強制先寫 `queue()`；頂層與鏈式 `wait(callback)` 都必須是 `run(callback)` 的同一函數別名。`style(url)` 載入單一 CSS，`style([url, ...])` 平行載入一組 CSS。`script(url)` 是循序步驟，`script([url, ...])` 會平行載入陣列內互不依賴的 scripts，並在全部完成後才繼續下一步；陣列內執行順序不保證。`vue(url)` 是選用的 Vue 2 SFC 佇列步驟，只有呼叫時才檢查 `System.import()`、Vue 2 完整版與 template compiler；`react(url)` 是選用的 React JSX 佇列步驟，只有呼叫時才檢查 `System.import()`、React 與提供 `createRoot()` 的 ReactDOM Client，再將 JSX 交給頁面既有的 SystemJS loader。fabLoader 不內建或自動下載 Vue、React 或 SystemJS，未使用 `.vue()`／`.react()` 時維持零外部依賴。Demo2 的 `systemjs.config.js` 必須保留為 Vue 設定，React JSX 設定獨立放在 `runtime.config.js` 並以 `// for react` 標記。舊 `.styles()` 不保留。`queue()` 只保留為明確建立空佇列的選用 API。先由正式版 Diagram Demo 與獨立 `demo2/` 試用，方向確認前不得併入 FabUI core。
- FabUI 語言檔採 EasyUI locale 的覆蓋模式：載入時除了註冊語言資料，也直接覆蓋已載入元件的英文預設訊息並自動切換既有控件，不需要再呼叫切換 API。fabLoader 必須保持獨立，不得辨識 FabUI 語言檔名或加入 FabUI 專用快取例外。
- `fabDom` 是位於 `src/fabdom/fabDom.js` 的零依賴 DOM helper 原始碼，只由 `build fabloader` 組合發佈，不提供獨立 build 或 dist；`build loader` 不得讀取或內建此檔案。若頁面沒有 `jQuery` 且 `$` 尚未定義，載入 fabloader 合併版時自動令 `$ === fabLoader.dom`，既有 jQuery 或其他 `$` 一律不得覆蓋。公開 jQuery-like 子集固定包含 collection `each()`／`get()`／`eq()`／`first()`／`last()`，內容與插入 `html()`／`text()`／`val()`／`append()`／`prepend()`／`before()`／`after()`／`load()`，attribute／property `attr()`／`removeAttr()`／`prop()`，CSS class `css()`／`addClass()`／`removeClass()`／`toggleClass()`／`hasClass()`，直接與 selector 委派事件 `on()`／`off()`，以及 `empty()`／`remove()`／`find()`／`closest()`／`parent()`／`children()`／`is()`。`append()`／`prepend()`／`before()`／`after()` 必須同時接受 HTML 字串、DOM Node、fabDom collection 與 NodeList；DOM Node 不得被字串化，多目標插入時前面的目標使用深層複本、最後一個目標使用原始節點。不得加入動畫、完整 Ajax、event namespace、trigger、custom event system、plugin 或完整 jQuery 相容層；`load()` 只橋接 fabLoader。HTML 字串不做 sanitization，只能直接使用固定或可信任內容。Demo2 只載入 fabloader 合併版，本機 jQuery 4 slim 保留為選用測試；jQuery 不得成為 FabUI 或 fabLoader 依賴。修改 fabDom 後必須自動執行 `npm run build:fabloader`，只輸出 `dist/fabLoader.js` 與 `dist/fabLoader.min.js`。
- `fabui` 是最上層 UI namespace，Browser global 發佈入口目前輸出 FabGrid、`fabui.collections`、`fabui.chart` 與其必要定義。
...
- FabGrid 與 `fabui.EditBox` 共用 `charcase` 定義；FabGrid 以 Column `charcase` 設定，`editor` 維持字串或 object 兩種寫法，EditBox 以頂層 option `charcase` 設定。`charcase` 套用於 `text`、`combo`、`color`，不得套用於 `date`、`time`、`number`；`upper`／`lower` 只轉換 ASCII 英文字母，中文、數字、空白與符號必須維持原值。FabGrid Column 未指定 editor 時，字串欄位預設使用 `text`。
- `fabui.EditBox` 的 `text`、`number`、`time`、`date`、`combo`、`color` 必須都公開可用的 `fix()`，並在焦點離開該控件時統一驗證及提交目前值。焦點移入該 EditBox 自己的 Date／Combo／Color popup 仍屬於同一次編輯，不得提前提交；從 popup 再移到控件外時才提交。
...
- `fabui.collections.CollectionView` 位於 `src/collections/collection-view.js`；FabGrid 與 Chart 的 `itemsSource` 同時接受 Array 與 CollectionView。共用 CollectionView 時，一般本機 FabGrid 的內建排序與篩選更新其 `items`，Chart 監聽 `collectionChanged` 依相同順序自動重繪；Grid row 與 Chart point 透過 `currentItem`、`currentPosition`、`currentChanged` 雙向同步，不得保留或重新加入 Chart `selectionSource`。FabGrid 初始化期間必須等 root DOM、view 與可見欄位都可用後才依 CollectionView `currentItem` 同步選取，不得在 DOM 建立前提前 render。Remote Grid 與 TreeGrid 維持各自既有排序流程，不得假設會改寫共用 CollectionView 的排序。
...
- FabGrid 同一時間只能有一個 active cell，cell editor 必須與該 active cell 使用相同座標；滑鼠單純離開 edit cell 時必須維持編輯不變，點擊其他 cell 時必須先提交目前 editor 值再切換，程式選取其他 cell 時仍取消目前編輯且不提交未完成值。
- Layout 的 `hide(region)`／`show(region)` 必須與 collapse／expand 分離：隱藏 edge region 時 Panel、Splitter、collapsed bar 與版面空間都必須移除，但不得改變原本的 collapsed 狀態、尺寸或 Panel instance；再次顯示時依原狀恢復。`center` 是必要區域，不得隱藏。
- Chart 公開 API 統一放在 `fabui.chart` namespace；`fabui.chart.Chart` 使用 pure JavaScript SVG rendering 提供直條圖、橫條圖與折線圖，`fabui.chart.Pie` 提供圓餅圖，並公開 `ChartType`、`Position`、`SelectionMode` 與包含 `ChartAnimation`、`AnimationMode`、`Easing` 的 `animation`。不得在 `fabui` 頂層重複公開 `Chart` 或 `Pie`。原始碼位於 `src/chart/` 並納入主 bundle。
- PivotChart 位於 `fabui.pivot.PivotChart`；直接監聽共用 PivotEngine 的 `updatedView`，只負責將 Pivot view 轉成既有 `fabui.chart.Chart` 的 categories／series，不得重新彙總原始資料或複製 Chart renderer。
- `fabui.EditBox` 與 FabGrid 的公開 editor 名稱統一為 `text`、`number`、`time`、`date`、`combo`、`color`。FabGrid 的時間欄固定使用 `dataType: 'string'` 搭配 `editor: 'time'`；`time` 是 editor type，不是 `dataType`。`editor: 'date'` 必須同時支援 `dataType: 'string'` 與 `dataType: 'date'`；字串日期必須明確設定 editor，Date 值可由 `dataType: 'date'` 自動選擇，也可明確設定 `editor: 'date'`。舊 `textbox`、`numberbox`、`timebox`、`datebox`、`combobox` 僅保留為相容別名，CSS class 名稱不必跟著改。Time EditBox 與 FabGrid time cell editor 預設 `mask: '99:99'`、`autoUnmask: false`、`spinner: false`，也支援 `99:99:99`；採一般 24 小時規則，分秒只接受 `00`–`59`，只有完整 `24:00`／`24:00:00` 可作為上限，Spinner 依游標所在時／分／秒段落調整。`fabui.EditBox` 是各 Box 實作的單一公開替代 class；內部原始碼統一放在 `src/editbox/`，不得再建立分散的 `src/*box/` 目錄，也不得在 `fabui` 頂層公開個別 Box class。`fabui.EditBox` 必須納入 `src/fabui.js`、`src/fabui.css`、`build/build.cjs` 與 `dist/fabui.*`，不得另行輸出 `dist/editbox.*`。
- FabGrid cell editor、Search Row 與 `fabui.EditBox` 的 icon descriptor 公開標準欄位統一為 `iconCls`、`title`、`ariaLabel`、`text`、`width`、`align`、`keepFocus`、`onClick`，並由 `src/editbox/editor-icons.js` 單次正規化；`className`／`iconClass`／`icon`、`label`、`click`／`handler` 只保留舊寫法相容，renderer 不得各自重複解析別名。`keepFocus` 未設定時行為等同 `true`；設為 `false` 時，`onClick` 同步交給外部 popup 的焦點不得觸發背景 Grid editor 提交、必填驗證搶焦點或重新聚焦，callback 結束後的後續失焦仍依原契約提交與驗證。
...
- 資料來源選項 `remote` 預設為 `false`；本機與 `remote: true` 的 `itemsSource` 都必須接受 Array 或 `fabui.collections.CollectionView`。遠端成功回傳 rows 時，Array 模式更新 Grid 的 Array 資料來源，CollectionView 模式必須更新同一個 instance 的 `sourceCollection`，讓共用 Chart 收到 `collectionChanged`；不得在 CollectionView 上重複套用已由後端處理的 Grid filter／sort。`remote: true` 已支援 GET／POST、Promise loader、遠端分頁、排序、全域搜尋與欄位篩選。
...
- FabUI jQuery 相容 Wrapper 位於 `packages/fabui-jquery`，只轉接 FabUI 公開元件 API、EasyUI 風格 plugin、`easyui-*` parser 與 `data-options`；不得複製元件內部行為或以 Wrapper 修補元件 Style。
...
- 所有公開 FabGrid 事件都必須由單一事件定義建立，並同時支援 constructor option、`grid.on()` 與 Wijmo-compatible `addHandler()`；三種 callback 簽名一律為 `(grid, eventArgs)`。每個 `eventArgs` 固定包含 `grid`、`type`、`cancel`，且 `eventArgs.grid === grid`。只有定義為可取消的事件可透過 `eventArgs.cancel = true` 或回傳 `false` 取消；完成後事件不得取消。相容事件別名必須共用同一 Event object，不得因別名重複派送同一次操作。
...
- FabGrid active cell editor 在焦點離開 Grid 時必須呼叫相同提交流程，讓 `text`、`number`、`time`、`date`、`combo`、`color` 都先驗證並寫回目前值；此提交不得把焦點搶回 Grid。焦點移入該 Grid 的 Date／Combo／Color editor popup 時仍屬於同一次編輯，不得提交或觸發 `lostFocus`；從 popup 再移到 Grid 外時才提交並觸發 `lostFocus`。
- FabGrid Column 的公開唯讀狀態只保留 `isReadOnly`，不提供 `readOnly` 相容名稱。只要 `grid.isReadOnly === true` 或 `column.isReadOnly === true`，該 cell 就不得由滑鼠、鍵盤或 `startEditing()` 開始編輯；Grid 可編輯不得覆蓋 Column 唯讀狀態。
- FabGrid Column 的 `isRequired` 預設為 `false`；只有明確設為 `true` 時才啟用內建必填驗證。`null`、`undefined`、空字串與純空白必須產生 `type: 'required'` 的 validation error 並加入 `invalidItems`，數值 `0` 與 boolean `false` 必須視為有效；必填驗證通過後才執行既有 `column.validate`，值修正後必須清除對應錯誤。
- FabGrid `invalidItems` 必須隨資料與欄位生命週期更新：資料列刪除、`setItemsSource()`、CollectionView／遠端資料取代、TreeGrid 刪除與跨 Grid 拖曳移出後，要移除不再屬於該 Grid 的同步及 pending 非同步錯誤；排序、篩選、分頁、列重排、欄位顯示與欄位重排後，所有保留項目的 row／column index 與 number 必須同步。只被篩選、分頁或群組收合隱藏但仍存在資料來源的列不得移除錯誤，僅將目前不可見的 row index 設為 `-1`。
- FabGrid Column `validate(args)` 必須提供本機 `args.isDuplicate(options?)`；它使用目前 binding、新值與完整本機 source，自動排除目前 item。`null`、`undefined`、空字串與純空白永遠允許重複，非空白字串先 trim 後比較且預設區分大小寫，`{ ignoreCase: true }` 時忽略大小寫；必須支援 Array、CollectionView、`observeItemsSource`、巢狀 binding 與 TreeGrid。
- FabGrid Column 的 `multiLine` 預設為 `false`；只有文字型 cell editor 在設為 `true` 時改用可承載換行內容的 `<textarea>`。此設定只允許 editor 保存輸入或貼入的多行值，不得改變目前 `editOnSelect` 的鍵盤契約；body cell 仍保持單行顯示與固定列高。
- FabGrid `hasFocus` 是唯讀 boolean property；目前 `document.activeElement` 位於該 Grid root 內時回傳 `true`，否則回傳 `false`。頁面有多個 Grid 時，同一時間最多只能有一個 Grid 的 `hasFocus` 為 `true`。
- 頁面有多個或巢狀 FabGrid 時，鍵盤事件只能由最後取得 pointer／焦點且為 event target 所屬最近 `.fg-root` 的 instance 處理；作用中 Grid 必須以跨 bundle 共用的 document marker 保存，且 Grid 消化方向鍵、Page、Home 或 End 導覽後必須停止事件冒泡，讓頁面轉送事件或焦點暫時回到頁面層時仍只處理一次，外層或其他 Grid 不得同步移動 selection。
- FabGrid `stopNavigation` 預設為 `false`；設為 `true` 時只封鎖使用者觸發的鍵盤導覽、cell／row 選取、滑鼠滾輪、觸控與捲軸捲動，必須保留目前 selected row 與 viewport。程式呼叫的選取、捲動與資料更新 API 不受限制；設回 `false` 後恢復使用者操作。
- FabGrid 批次更新 API 使用可巢狀的 `beginUpdate()`／`endUpdate(shouldInvalidate?)`、`deferUpdate(callback)` 與唯讀 `isUpdating`。批次期間只延後 View render／refresh／invalidate，不得吞掉資料、排序、篩選或選取等既有事件；只有最外層 `endUpdate()` 可執行一次累積更新，`endUpdate(false)` 不得自動更新，`deferUpdate()` 在 callback 發生例外時仍必須恢復更新並繼續拋出例外。
...
- `selectionMode` 支援 `Cell` 與連續矩形 `CellRange`；後者支援滑鼠拖曳、列號點擊／拖曳選取所有可見欄位、`Shift + Click`、`Shift + 方向鍵`與 TSV clipboard copy。CellRange 使用資料列選取背景，active cell 保留一般 cell 背景及與單格選取相同的完整四邊框，範圍四周外框必須共用 `activeCellBorder` 粗細；設為 `0` 時不顯示外框，且不得因 `highlightActiveRow` 額外填滿 active row。Default 與其他 16 組主題都必須明確定義 `--fg-range-selected-bg`、`--fg-range-active-bg`、`--fg-range-border`，分別沿用該主題的列選取背景、cell 背景與選取邊框色。CellRange 雙擊必須由同一資料格的連續 pointer 操作成立，`pointercancel` 不得觸發雙擊，完成 pointer 選取後不得再由 click 重複套用 selection 或 render。`highlightActiveRow` 預設為 `true`，只控制 `Cell` 模式的 active row 背景，不得隱含改變多選列；active cell 邊框預設為 1px，`setRowHeaderWidth(width)` 可在 runtime 調整列號欄寬並自動 refresh。
- FabGrid `autoSearch` 預設為 `false`；設為 `true` 時，可在非編輯狀態對 active column 輸入 500ms 內累積的前綴字串，從目前列往下循環尋找並移動到符合列。首字元從下一列開始，多字元找不到時退回最後輸入字元重新尋找；`caseSensitiveSearch` 預設為 `false`，設為 `true` 才區分大小寫。
- `select(row, col?)` 維持 active cell API，不得改成 `selectRow` 別名；明確傳入的 `col` 對應完整 `grid.columns` index 並計入隱藏欄位，省略時預設選取第一個可見欄。這個公開欄座標必須可直接和 `formatItem` 的 `e.col` 及 `editRange.col` 比較。不論是否傳入 `col`，若目標 row 未完整顯示，必須自動捲動，並盡量將該 row 對齊 Grid 可視區第一列。
- `startEditing()` 必須同時保留 FabGrid 既有 `startEditing(row, col, options?)` 與 Wijmo-compatible `startEditing(fullEdit?, row?, col?, focus?, event?)`。沒有 row／col 時使用目前 active cell；Wijmo overload 明確傳入的數字 `col` 對應完整 `grid.columns` index 並計入隱藏欄位，也接受 Column `name`／`binding`／`header` 字串。隱藏或不存在的目標 cell 回傳 `false`；`focus === false` 時建立 editor 但不得搶走焦點。`fullEdit` 必須傳入編輯狀態與起始事件參數，但 FabGrid 既有鍵盤導覽仍依本專案的 `editOnSelect` 與 Enter／Tab 契約處理。
- 只要 Cell editor 正在編輯，`Enter`／`Shift+Enter` 會向右／向左移至下一個可編輯 cell；列尾接續下一列第一個可編輯 cell，列首則可回到上一列最後一個可編輯 cell，且不受 `editOnSelect` 影響。`editOnSelect: true` 時，`Tab`／`Shift+Tab` 維持可跨列的向右／向左搜尋，`ArrowUp`／`ArrowDown` 維持同欄連續編輯。上述連續移動不得套用公開 `select()` 的首列對齊行為；active editor 超出可視區後每次只捲動所需列數，並保持在可視區頂部或底部邊界。`editOnSelect: false` 且允許編輯時，在 active cell 輸入可用字元必須自動開始編輯並以該字元取代原值；`Tab`／`Shift+Tab` 只提交並結束編輯，未由 Spinner 或已開啟 Popup 接管的上下左右方向鍵保留 editor 的原生字元游標行為。
- FabGrid Column `stayOnInvalid` 預設為 `false`；設為 `true` 時，該欄其他同步／非同步驗證在通過前都不得離開 editor。Column `isRequired: true` 的必填錯誤固定套用相同行為，不受 `stayOnInvalid` 值影響。失敗值不得寫回資料，Enter、Tab、方向鍵、滑鼠點擊其他 cell 與焦點離開 Grid 都必須保留原 editor；仍觸發 `cellEditEnding`，但只有驗證通過並提交時才觸發 `cellEditEnded`。非同步驗證等待期間不寫入資料，輸入改變時忽略舊結果並於下次離開重新驗證，reject 視為驗證失敗；`Escape` 可取消並忽略未完成結果。`stayOnInvalid: false` 時維持非同步提交後驗證。Grid 不提供同名選項。
- FabGrid 唯讀 `editRange` 在編輯中回傳目前 cell 的 `{ row, col, row2, col2 }` 快照，其中 `row2 === row`、`col2 === col`，`col`／`col2` 必須對應 `grid.columns` 的原始 index 並計入隱藏欄位，以便直接和 `formatItem` 的 `e.col` 比較；未編輯時回傳 `null`，修改回傳物件不得影響 Grid 內部編輯狀態。Editor 必須等編輯重繪與定位都成功後才顯示；若 `formatItem` 或定位流程發生例外，必須清除編輯狀態並隱藏 editor 與 icon host，不得留下未定位的原生輸入框破壞 Grid 版面。
- FabGrid Column 不提供 `minWidth`。Column 未設定 `width` 時，初始欄寬使用全域 `columnMinWidth`（預設 `20px`）；明確設定的初始 `width` 可小於此值，滑鼠拖曳調寬與 `autoSizeColumn()` 仍以 `columnMinWidth` 為全域下限。
...
- Filterable Header 文字必須垂直置中；寬度足夠時保留既有左側 `6px` 與右側 `24px` 間距，sort icon 緊接文字，Filter icon 位於最右側。內容寬度不足時左右外側間距降為 `2px`，Filter icon 仍固定顯示於右側；有排序時 sort icon 移到 Filter icon 下方，文字區必須避開共用圖示區。未啟用 filter 時不得建立 Filter icon 或保留其空間，Header 文字與 inline sort icon 可以使用完整欄寬。漏斗圖形在 hit area 內的左側內距為 `2px`；Default 與其他淺色主題切換為運算符 icon 後必須使用一般字重的 `blue`，`black` 與 `dark-hive` 深色主題改用白色。可見 Filter icon 必須維持獨立且高於文字的 hit area，點擊只開啟篩選選單，不得觸發排序或欄位拖曳；Header 右邊界的 resize handle 必須高於 Filter icon，確保拖曳調寬與雙擊 AutoFit 可用。
- Header sort 符號下緣必須與文字行框下緣對齊；一般欄位、關閉 filtering 或寬度足夠時維持緊鄰欄位文字。窄 Filterable Header 的 sort icon 必須位於 Filter icon 下方，多欄排序序號與 sort icon 並排顯示。
...
- `filterRules` 不得因目標欄位 `visible: false` 而失效；本機初始化與 `setFilterRules()` 必須比對隱藏欄位，遠端仍須傳送該欄規則。欄位顯示狀態只控制畫面，不得略過既有篩選條件。
- 本機與遠端 `filterRules` 必須接受相同的標準 operator 與相容符號：`..%`、`%..%`、`%..`、`!..%`、`!%..%`、`!%..`、`>=`、`>`、`<=`、`<`、`<>`、`=`，分別正規化為 `starts`、`contains`、`ends`、`not-starts`、`not-contains`、`not-ends`、`gte`、`gt`、`lte`、`lt`、`ne`、`eq`。兩種模式也都支援不分大小寫的 `in` 與陣列／逗號分隔字串值；本機使用該欄既有 `eq` 語意比對任一非空白項目，自訂未知 `op` 仍僅供遠端處理。中文輸入法 composition 期間的 Search Row 方向鍵與 keyCode `229` 必須保留給候選字操作，不得切換 Grid 焦點；composition 結束後才依完成文字套用篩選。
- Search Row input 的 `searchDelay` debounce 預設為 `400ms`，本機與 `remote: true` 共用；每次 input 必須取消前一次 timer 並重新計時，設為 `0` 時才可立即套用。
- Search Row 左側的 RowHeader top cell 與 selection top cell 不得綁定 mouse double-click 動作；雙擊不得清除 Search Row、Quick Search 或觸發遠端查詢。
- `remote: true` 的 Search Row 查詢必須使用非阻擋式背景 loading，等待期間保留目前 rows 並允許操作 Grid 與繼續輸入，只在 Grid 中央顯示小型 loading 提示。新 Search Row 查詢觸發時，內建 Fetch 必須中止上一個 request；自訂 `loader` 無法由核心強制中止時也必須放棄舊回應，只允許最新查詢更新資料與觸發完成事件。初次載入、分頁、排序與其他遠端載入仍使用全 Grid loading mask。
...
- Chart 公開結構固定為 `fabui.chart.Chart`、`fabui.chart.Pie`、`fabui.chart.ChartType`、`fabui.chart.Position`、`fabui.chart.SelectionMode` 與 `fabui.chart.animation`；animation namespace 固定公開 `ChartAnimation`、`AnimationMode` 與 `Easing`。原始碼位於 `src/chart/chart.js` 與 `src/chart/chart.css`，並納入 FabUI 主 bundle；不要把核心 Chart 行為只寫在 Demo，也不得在 `fabui` 頂層公開 `Chart`／`Pie`。
- `new fabui.chart.animation.ChartAnimation(chart, options?)` 只接受 `fabui.chart.Chart`／`Pie` instance，公開唯讀 `chart`、`animationMode`、`axisAnimation`、`duration`、`easing`、`animate()` 與 Wijmo-style `ended.addHandler()`／`removeHandler()`；建立後附加到該 Chart，refresh 時沿用設定，Chart dispose 時必須解除 timer 與參照。
...
- 一般本機 Grid 與 Chart 需要同步排序、篩選與選取時必須共用 `fabui.collections.CollectionView`；不要在 Demo 維護第二份資料、監聽排序或 `filterChanged` 手動更新 Chart，或使用已移除的 `selectionSource`。Remote Grid 與 TreeGrid 不適用此排序同步契約。
- Array 仍可直接指定給 Grid 或 Chart `itemsSource`。Chart 預設以 `observeData: true` 偵測 Array 綁定欄位變動；CollectionView 則以 `collectionChanged` 與 `currentChanged` 驅動重繪與選取。
...
1. 資料顯示
   ...
   - 支援 `binding`、`header`、`width`、`align`、`dataType`。
...
4. Frozen columns
   ...
   - `frozenColumns` 不可大於完整 columns 總數，超過時應 clamp；hidden columns 仍計入指定範圍。
...
- `dist/theme/fabui.<theme>.css` 與 `dist/theme/fabui.<theme>.min.css` 提供 Default 以外 16 組固定 selector 覆蓋樣式，不輸出重複的 Default Theme 檔，也不保留 `fabgrid.*.css` 舊檔名。
...
- Vue wrapper build 只輸出壓縮版 `packages/fabgrid-vue/dist/fabgrid-vue2.min.js`，並將 `vue.min.js` 與 browser global `fabgrid-vue2.min.js` 同步到 `dist/wrapper`；不得產生未壓縮 wrapper、保留舊 `fabgrid-vue.min.js`，或併入 `dist/fabui.*` 主 bundle。
- Vue Demo 固定使用 `demo/grid-vue2.html`，依序載入 `dist/wrapper/vue.min.js`、`dist/fabui.min.js` 與 `dist/wrapper/fabgrid-vue2.min.js` browser global，再由 SystemJS runtime loader 動態載入 `demo/js/grid-vue2.vue`。目前不提供獨立 production HTML；不得恢復已移除的 `demo/vue2-grid-app.html`，除非使用者明確要求新的發布流程。
...
- FabUI jQuery 相容 Wrapper 位於 `packages/fabui-jquery`，公開 Layout、Panel、Tabs、Window／Dialog、Form、EditBox、ValidateBox、FileBox、Button、Menu、MenuButton、Tooltip、Tree、SwitchButton 等 EasyUI 風格 plugin，並在 FabGrid Wrapper 已載入時提供 `datagrid` 相容入口。
- `npm run build:fabui-jquery` 只輸出 `packages/fabui-jquery/dist/fabui-jquery.{js,min.js}` 與 `dist/wrapper/fabui-jquery.min.js`，不得改寫 `dist/fabui.*`；`build:all` 透過此獨立命令納入 FabUI jQuery wrapper。
- `$.parser.parse(context)` 只解析 `easyui-*` class 與安全的 `data-options` 資料，不執行其中的 JavaScript；事件 callback 使用程式式 options。
...
- jQuery wrapper build 完整輸出到 `packages/fabgrid-jquery/dist`，browser minified 版本另同步到 `dist/wrapper/fabgrid-jquery.min.js`；不得併入 `dist/fabui.*` 主 bundle。
...
- 每次使用者明確要求 build 時，依「開發工作流程」定義的 `build`／`build fabui`、`build lite`、`build diagram`、`build gantt`、`build scheduler`、`build htmleditor`、`build fabloader`、`build loader`、`build theme`、`build locale`、`build clear`、`build all`、逗號分隔多範圍或上述命令的 `min` 形式執行對應操作與檢查；逗號左右不得有空白，`min` 必須套用到清單中的每個範圍。Fabloader 與 Loader 都不納入逗號分隔多範圍或 `build all`；`build all` 必須包含 Vue 2、FabGrid jQuery 與 FabUI jQuery wrapper。
