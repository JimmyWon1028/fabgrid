...
`fabui.Diagram` 是 FabUI core 內建的 pure JavaScript SVG 圖表設計器。介面參考 DevExtreme Diagram Overview 的工具列、圖形工具箱、畫布與屬性區配置，但不依賴 DevExtreme、jQuery 或其他 runtime；內部操作按鈕重用 `fabui.Button`，搜尋與屬性欄位重用 `fabui.EditBox`，畫布右鍵選單重用 `fabui.Menu`。上方工具列由左至右分為 JSON 下載／載入、輸出、復原／編輯、檢視控制四組，中間以垂直線分隔；投影片展示位於右下角浮動檢視工具列的全螢幕左側，匯出 PNG、匯出 SVG、列印與投影片展示同時保留於畫布右鍵選單。紙張小於 viewport 時會保持水平與垂直置中，放大超出可視區域後仍可正常捲動。

工具箱的「一般」、「流程圖」、「DFD」與「組織圖」分類標題使用 `fabui.Button`，以 20px 箭頭清楚表示疊合或展開；切換語系及搜尋圖形時會保留目前狀態。拖曳工具箱或屬性面板最上方標題可將面板變成繪圖工作區內的浮動面板，也可拖至左側或右側重新停靠；兩個面板可各自停靠，包含同時停靠左側或右側。同側停靠支援兩種模式：`tabs` 重用 `fabui.Tabs`，工具箱／屬性標籤固定置於下方並只顯示目前選取面板；`stacked` 則將工具箱置於上方、屬性置於下方並同時顯示。兩個面板停靠時可由內側邊緣調整寬度，浮動時可由左右邊調寬、上下邊調高；聚焦邊緣後也可用對應方向鍵調整。47 種圖形預覽重用 Diagram SVG shape renderer；一般、流程圖與 DFD 預覽維持空心輪廓，組織圖的人像預留區依圖形顯示在左側、右側或上方。DFD 提供 Entity、Process、Data Store 節點與 Data Flow 工具；Data Flow 會啟用帶箭頭的弧線 connector 模式。
...
| `type` | `orthogonal`、`straight` 或 `curved`。 |
| `controlX`／`controlY` | 拖曳連線後保存的控制點座標；直線形成折點、直角線移動中段、弧線調整彎曲方向與弧度。 |
| `text` | 連線標籤。 |
...
連線可直接以滑鼠拖曳；選取後會顯示中段控制點。拖曳 `curved` 連線會移動二次貝茲曲線控制點以改變弧度，拖曳結果會保存到 JSON 並支援 undo／redo。
...
| `toolboxStateKey` | `''` | 指定 `localStorage` key 後，會保存分類展開／順序、工具箱與屬性面板的版面狀態、同側排列模式、作用中 tab、紙張設定，以及檢視工具列的拖曳位置；重新載入時恢復最後狀態。舊版內容仍可讀取；空字串不保存。 |
...
「唯讀」按鈕會呼叫 `setReadOnly()` 鎖定或解除鎖定做圖區，pressed 狀態與 `options.readOnly` 同步。唯讀時主工具列仍保持顯示，以便解除唯讀；復原、重做、刪除、清除、載入、連線、工具箱與屬性等編輯操作會停用，工具箱及屬性區會隱藏，但縮放、符合內容、格線、全螢幕、下載、右鍵匯出與投影片展示等檢視操作仍可使用。
...
- 右下角檢視工具列可從縮放百分比或工具列空白處拖曳，並限制在 Diagram 範圍內；純文字「投影片展示」固定在「全螢幕」左側且不顯示 icon，按鈕點擊功能保持不變。設定 `toolboxStateKey` 後，重新載入會連同兩個面板版面、工具箱寬度與分類狀態，還原檢視工具列位置。
- 拖曳工具箱右側分隔把手可調整寬度；把手取得焦點後，左右方向鍵每次調整 10px，搭配 `Shift` 時每次調整 30px。
- 上方工具列與做圖區右鍵選單都可匯出 PNG、匯出 SVG 及列印目前紙張；浮動檢視工具列與右鍵選單都可選擇「投影片展示」，展示期間兩處文字都會切換為「離開投影片展示」。
- 右下角「全螢幕」顯示完整 Diagram 編輯器；進入時先自動符合頁面，之後可繼續使用「−／＋」手動縮放。做圖區右鍵「投影片展示」只顯示 viewport，將紙張依螢幕尺寸放大置中。兩者離開後都會還原原本的縮放與捲動位置。
- 在畫布空白處雙擊會開啟使用 `fabui.Window`、`fabui.EditBox` 與 `fabui.Button` 組成的紙張設定視窗；可設定紙張尺寸、方向與最小 5px 的吸附間距，套用後會立即更新格線與 snap，並保存在 `getData()`／匯出 JSON 的 `page.gridSize` 欄位。有效的 `toolboxStateKey` 會額外記住完整 `page`，重新載入時還原；載入含 `page` 的 JSON 也會更新這份記憶。
