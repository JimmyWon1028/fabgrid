...
`fabui.SwitchButton` 是 FabUI core 內建的 pure JavaScript 切換按鈕。API、滑動方式與視覺參考 jQuery EasyUI SwitchButton 1.11.5，但不依賴 jQuery 或 EasyUI runtime。16 組主題分別對應本機 `res/themes/<theme>/switchbutton.css`，正式 source、Demo 與 build 不會依賴 `res/`。
...
| `theme` | `'inherit'` | 繼承外層 `fg-theme-*`，或指定 16 組內建主題。 |
...
16 組主題為：`default`、`bootstrap`、`cupertino`、`material`、`material-blue`、`material-teal`、`metro`、`metro-blue`、`metro-gray`、`metro-green`、`metro-orange`、`metro-red`、`sunny`、`pepper-grinder`、`dark-hive`、`black`。
