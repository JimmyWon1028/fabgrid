...
`fabui.Calendar` 是 FabUI core 內建的 pure JavaScript 月曆元件。API 與 Default 視覺參考 jQuery EasyUI Calendar，但不依賴 jQuery 或 EasyUI runtime。它和 Date EditBox、FabGrid 日期 editor 共用同一個 `DatePopup` 日曆 renderer、月份選單、日期驗證、農民曆與 17 組 theme 樣式。
