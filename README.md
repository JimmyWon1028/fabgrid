...
FabGrid 的 Fab 代表 fabulous，是一個以效能為優先的 pure JavaScript data grid。核心目標是用簡單、可預測、可打包引用的方式，支援大量資料表的雙向 virtualization、凍結欄、選取、編輯、匯出與接近 Wijmo FlexGrid 的常用 API 形狀。

目前 demo 使用 `1000 x 30` 測試資料，預設左側凍結 2 欄、右側凍結 1 欄。

## 線上 Demo

[https://jimmywon1028.github.io/fabgrid/demo/](https://jimmywon1028.github.io/fabgrid/demo/)

## 工作進度

- [2026-07-11 FabGrid 改名紀錄](./worklogs/2026-07-11.md)
...
- 核心不依賴 Vue、React 或後端。
- demo 使用打包後的 `dist/fabgrid.min.js` 與 `dist/fabgrid.css`，用來驗證可直接被其他專案引用。
...
- Header filter icon 可切換搜尋運算符，文字欄支援開頭、包含、結尾與反向比對，數字與 datebox 欄支援 `>=`、`>`、`<=`、`<`、`!=`、`=`。
- 搜尋列 input 會沿用欄位對齊，datebox / combobox 欄會沿用內建 editor icon 與輸入行為，但不做 validate。
- Editor 與搜尋列 input 都支援自訂 icon button，可用 `icons: [{ iconCls, onClick }]` 定義。
- 欄寬拖曳調整。
- CSV 匯出。
...
- 主題 CSS，來源為 `src/styles/themes/fabgrid.<suffix>.css`，並可依 `src/styles/my.wijmo.*.css` 的後綴產生對應主題。
...
```html
<link rel="stylesheet" href="./dist/fabgrid.css">
<div id="grid"></div>
<script src="./dist/fabgrid.min.js"></script>
<script>
...
```js
import { FabGrid } from './dist/fabgrid.esm.js';
...
FabGrid 核心不內建完整語言包，語言檔會獨立輸出到 `dist/locales/`，依需要載入即可。若用 `<script>` 引用，先載入核心，再載入需要的語言檔：

```html
<script src="./dist/fabgrid.min.js"></script>
<script src="./dist/locales/fabgrid-locale.zh-TW.js"></script>
<script src="./dist/locales/fabgrid-locale.en.js"></script>
```

目前提供：

- `fabgrid-locale.en.js`
- `fabgrid-locale.zh-TW.js`
- `fabgrid-locale.zh-CN.js`
...
ES module 版本會匯出目前的語系 registry，語言資料仍建議用獨立語言檔載入或自行呼叫 `FabGrid.addLocale()`：

```js
import { FabGrid, FabGridLocales } from './dist/fabgrid.esm.js';
```

## 主題

FabGrid 主題來源放在 `src/styles/themes/fabgrid.<suffix>.css`，build 後會輸出到 `dist/themes/`。如果 `src/styles/my.wijmo.<suffix>.css` 存在，build 也會參考同後綴的 Wijmo 主題色票產生對應 FabGrid 主題：

```txt
dist/themes/fabgrid.default.css
dist/themes/fabgrid.material-blue.css
dist/themes/fabgrid.dark-hive.css
```
...
```html
<link rel="stylesheet" href="./dist/fabgrid.css">
<link rel="stylesheet" href="./dist/themes/fabgrid.dark-hive.css">
```
...
grid.setColumnSearch('name', 'Customer');
grid.setColumnSearchOperator('amount', 'gte');
grid.clearColumnSearch();
grid.clearSearchConditions();
...
grid.exportExcel('fabgrid-demo.xlsx').then(function() {
...
dist/
  ...
  locales/
    fabgrid-locale.en.js
    fabgrid-locale.en.min.js
    fabgrid-locale.zh-TW.js
    fabgrid-locale.zh-TW.min.js
    fabgrid-locale.zh-CN.js
    fabgrid-locale.zh-CN.min.js
  themes/
    fabgrid.<suffix>.css
    fabgrid.<suffix>.min.css
```
...
### 10. 日常開發要分清楚 source-mode 與 dist-mode

日常修改先用 `demo/dev.html` 測 `src/fabgrid-core.js` 與 `src/styles/*`。只有在使用者要求編譯時才更新 `dist`。這可以避免每個小修正都混入大量 build 產物，也能更快確認 source 行為。

source-mode 與 dist-mode 都要注意 query version。`demo/dev.html` 改 source 後要更新 source query；`demo/index.html` 改 dist 後才更新 dist query。

### 11. 新 UI 文字要一起補 locale

搜尋列、filter popup、datebox / combobox popup、validation message、ARIA label 都屬於使用者會看到或輔助工具會讀到的文字。新增這類功能時要同步檢查：

- `src/locales/fabgrid-locale.en.js`
- `src/locales/fabgrid-locale.zh-TW.js`
- `src/locales/fabgrid-locale.zh-CN.js`
- demo 自己的 `DEMO_LOCALES`

如果只補核心 locale，demo popup 或 demo button 仍可能出現混合語言。

### 12. Popup menu 要共用同一套視覺規則

目前 filter operator menu 的樣式已定稿：左側 icon 欄、icon 後有分隔線、選項列緊湊、hover 有清楚回饋。後續新增 popup menu 時應沿用這個規則，避免每種 popup 都長得不一樣。

### 13. Filter 會改變 view，必須同步清理 editor popup

搜尋或 filter 後，原本正在編輯的 row 可能已經不在 view 裡。此時 editor、datebox panel、combobox panel 都要關閉，不然會留下漂浮在空白 grid 上的 popup。這類 view-changing 操作要一起檢查 editor state。
