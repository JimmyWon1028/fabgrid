...
| Display customization | Provides Column `cssClass`, formatters, `formatItem`, `cellTemplate`, `FabGrid.CellMaker.makeLink()` link templates, Header styles, Row/GroupRow types, and event APIs. Body cells keep their normal spacing when content fits and remove only the right padding when content is clipped. |
...
| Header context menu | The popup may extend beyond the Grid's bounds and is kept inside the browser viewport. |
