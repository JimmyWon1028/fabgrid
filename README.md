...
<script>
  ...
  var grid = new fabui.FabGrid('#grid', {
    itemsSource: rows,
    columns: [
      { binding: 'id', header: 'ID', width: 80, align: 'center', dataType: 'number', allowResizing: false },
...
| Selection and clipboard | Supports Cell, CellRange, the read-only `selectedRow` active row, single-selection `unselectRow()`, `select()` with full `grid.columns` indexes, `selectedRowChanged`, multiple row selection, mouse dragging, whole-row range selection through row headers, Shift extension, keyboard navigation, and TSV copy. Multi-select row checkboxes only update checked rows and do not move the active cell; the select-all checkbox also avoids raising a single-row `selectedRowChanged`. With `autoSearch: true`, printable keys search the active column by a 500ms accumulated prefix, wrap through rows, and optionally honor `caseSensitiveSearch`. Clicking a RowHeader selects the whole row while preserving the RowHeader's original appearance; `Ctrl/Cmd + C` copies every visible column in that row. Every public event `e.col` uses the full `grid.columns` index including hidden columns; `e.viewCol` exposes the visible index when needed. CellRange borders use `activeCellBorder`. `stopNavigation` can pause user selection and scrolling while keeping programmatic APIs available. |
...
| Display customization | Provides Column `cssClass`, formatters, `formatItem`, `cellTemplate`, `FabGrid.CellMaker.makeLink()` link templates, Header styles, Row/GroupRow types, and event APIs. Narrow Headers reduce their outer padding to `2px`; Filter icons remain visible and keep the same position when enabled, while Headers without filtering reuse that space. Sort icons stay beside the label when possible and move below the Filter icon when the available width is insufficient without changing a fitting label's left padding. Row hover updates existing rendered cells directly instead of rebuilding the visible rows. Body cells keep their normal spacing when content fits and remove only the right padding when content is clipped. |
| Import and export | Supports JSON, CSV, and XLSX. Excel exports support custom sheet names and multi-sheet workbooks from multiple Grids while preserving data-cell background and text colors from the normal formatting pipeline, frozen panes, filters, groups, Footers, and hidden columns. Exported Footer cells use a gray background with black text. Numeric Columns and numeric Footer values keep their numeric cell type and thousands-separator format; when `precision` is omitted, integer cells do not display a trailing decimal point while actual decimal values retain a decimal format. |
| Popups | Context menus, filters, column choosers, and editor popups close with `Escape` or an outside click. Page-level filter and context-menu popups automatically rise above the Grid's containing window stacking level. With `filterMode: false`, Clear Filter is hidden. Configured `rowGroups` add an expand-all/collapse-all state item before Excel export. Row Header and fullscreen entries are independently controlled by `showRowHeaderMenu` and `showFullscreenMenu`, both hidden by default. |
...
For a `dataType: 'number'` Column, finite numeric strings are normalized internally for display, sorting, Search Row and Excel-like filtering, aggregates, and XLSX export. The Grid does not mutate `itemsSource`, and unformatted `getCellData()` still returns the original value. Invalid text remains visible but is excluded from numeric operations; a committed number edit writes a Number.
...
| Search Row keyboard | `Enter`/`Tab` moves to the next column and `Shift+Enter`/`Shift+Tab` moves to the previous column. Arrow keys move between the Search Row and Grid, except during IME composition when candidate navigation remains owned by the input method. On pages with multiple or nested Grids, only the Grid that most recently received pointer or focus interaction is affected. Once a Grid consumes a navigation key, propagation stops so the action is handled only once. |
| Excel-like filter | The popup may extend beyond the Grid's height to use more of the browser viewport. Local candidate values follow filters from other columns while ignoring the target column's own filter. When the candidate search has text, Apply keeps only matching checked values. In remote mode, reopening a column preserves the complete candidate list and current checked state. |
| Header context menu | The popup may extend beyond the Grid's bounds, is kept inside the browser viewport, and automatically rises above the Grid's containing window stacking level. |
...
| Feature | Description |
| --- | --- |
| Custom icons | Use `icons: [{ iconCls, title, ariaLabel, text, width, align, keepFocus, onClick }]`. `keepFocus` defaults to `true`; set it to `false` when `onClick` must synchronously hand focus to an external popup without prematurely committing or refocusing the Grid editor. `iconCls` may be an external CSS class that defines `background` or `background-image`; it does not require `:root` or `!important`. |
