...
- `columns` 欄位定義，支援 `binding`、`header`、`width`、`minWidth`、`align`、`dataType`、`formatter`、`mask`、`maskValueIncludesLiterals`、`readOnly`。
- 雙向 virtualization。
...
- 欄寬拖曳調整。
- CSV 匯出。
- Excel 匯出，包含欄寬、凍結窗格、filter、grid border、header/cell 背景與文字顏色。
...
- 編輯模式：預設關閉，可用 demo 的「編輯」checkbox 或 `setEditMode(true)` 開啟
- 唯讀欄位：demo 的 `ID`、`客戶名稱`、`區域`、`狀態`、`日期` 共 5 欄設為 `readOnly: true`
...
- CSV / Excel 相關輸出。
- Excel 是否保留 grid border、header fill、cell color/background、frozen pane。
