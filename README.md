...
日常修改先用 `demo/dev.html` 測 `src/fastgrid-core.js` 與 `src/styles/*`。只有在使用者要求編譯時才更新 `dist`。這可以避免每個小修正都混入大量 build 產物，也能更快確認 source 行為。
...
- `src/locales/fastgrid-locale.en.js`
- `src/locales/fastgrid-locale.zh-TW.js`
- `src/locales/fastgrid-locale.zh-CN.js`
