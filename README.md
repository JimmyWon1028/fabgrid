...
| Import and export | Supports JSON, CSV, and XLSX. Excel exports support custom sheet names and multi-sheet workbooks from multiple Grids while preserving formatting, frozen panes, filters, groups, Footers, and hidden columns. Numeric Columns keep their numeric cell type and thousands-separator format; when `precision` is omitted, integer cells do not display a trailing decimal point while actual decimal values retain a decimal format. |
