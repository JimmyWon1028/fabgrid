...
function parseFabEditBoxDataOptions(value) {
  var text = String(value == null ? '' : value).trim();
  var options = {};
  if (text.charAt(0) === '{' && text.charAt(text.length - 1) === '}') {
    text = text.slice(1, -1);
...
function toFabEditBoxJQueryEventName(value) {
  return String(value || '').toLowerCase();
}

function isFabEditBoxPublicMethod(instance, name) {
  return Boolean(
    instance &&
    typeof name === 'string' &&
    name.charAt(0) !== '_' &&
...
function createFabEditBoxJQuery($, fabui) {
